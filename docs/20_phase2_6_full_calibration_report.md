...
校准三问和主线选择产生的状态变化（doubt/control/obedience/anomaly）目前缺乏运行时查看手段，玩家和开发者在试玩中无法直观验证积累效果。建议后续加入轻量 debug overlay（如 F1 快捷键显示四值浮层）或 `print()` 日志输出，降低验收和调试门槛。

### StoryLabel 闪烁效果待调整（阶段 2.5 遗留，已完成记录，本阶段未修改）

两项已记录在 `docs/19_phase2_5_feedback_polish_report.md` 但尚未实现的优化：

1. **入场闪烁晃眼**：进入车站时全屏 alpha 闪烁（1.0→0.65→1.0）视觉冲击过强，需改为**从右向左的墨水屏逐行/逐块刷新效果**（非全屏闪），减少突兀感
2. **每轮文本闪烁需更强更慢**：当前参数（0.65→1.0，0.08s + 0.12s）变化幅度偏小、速度偏快。需调大幅度和延长时长，具体参数待后续测试确定

留存后续 UI 打磨阶段（阶段 2.8 或独立优化轮次）处理。
